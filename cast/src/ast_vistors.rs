...
pub trait ASTVisitor: Sized {
    // Call `visit_mut` on all children of a node.
    fn visit_node_children<T: ASTNodeImpl>(&mut self, node: &T) {
    ...
    // Base function called when `x.visit` is called.
    // Will dispatch automatically to the right function.
    fn visit_generic_node(&mut self, node: &ASTNode) {
        match node {
            ASTNode::IntLiteral(node) => self.visit_int_literal(node),
            ASTNode::FloatLiteral(node) => self.visit_float_literal(node),
            ASTNode::StringLiteral(node) => self.visit_string_literal(node),
    ...
    fn visit_int_literal(&mut self, node: &ASTNodeIntLiteral) {
        self.visit_node_children(node);
    }

    fn visit_float_literal(&mut self, node: &ASTNodeFloatLiteral) {
        self.visit_node_children(node);
    }

    fn visit_string_literal(&mut self, node: &ASTNodeStringLiteral) {
        self.visit_node_children(node);
    }

    fn visit_char_literal(&mut self, node: &ASTNodeCharLiteral) {
        self.visit_node_children(node);
    }

    fn visit_label_expr(&mut self, node: &ASTNodeLabelExpr) {
        self.visit_node_children(node);
    }

    fn visit_binop_expr(&mut self, node: &ASTNodeBinopExpr) {
        self.visit_node_children(node);
    }

    fn visit_block_statement(&mut self, node: &ASTNodeBlockStatement) {
        self.visit_node_children(node);
    }

    fn visit_return_statement(&mut self, node: &ASTNodeReturnStatement) {
        self.visit_node_children(node);
    }

    fn visit_var_decl_statement(&mut self, node: &ASTNodeVarDeclStatement) {
        self.visit_node_children(node);
    }

    fn visit_label_type(&mut self, node: &ASTNodeLabelType) {
        self.visit_node_children(node);
    }

    fn visit_decls_list(&mut self, node: &ASTNodeDeclsList) {
        self.visit_node_children(node);
    }

    fn visit_function_decl(&mut self, node: &ASTNodeFunctionDecl) {
        self.visit_node_children(node);
    }
}
